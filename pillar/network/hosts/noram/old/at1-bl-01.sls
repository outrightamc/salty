proxy:
  host: at1-bl-01.net.arkadin.lan
  driver: junos
...
# physical > name, description, state: enable/disable, lacp for aggregated interface, mtu
  ...
  - { name: et-0/0/3, descr: 'X:at1-bl-02:et-0/0/3:b2b', state: enable, lacp: ae0 }
...
# subinterface or ae > unit vlan ID, description, state: enable/disable, mpls: true/false
  - { name: ae0, descr: 'X:at1-bl-01:ae0.0:b2b', mtu: 9234, unit: {id: 0, address: 172.22.4.0/31, mpls: true} }
  - { name: ae1, descr: 'X:at1-lf-01:ae1.0:fabric', mtu: 9234, unit: {id: 10, address: 172.22.4.2/31} }
  - { name: ae2, descr: 'X:at1-lf-02:ae1.0:fabric', mtu: 9234, unit: {id: 10, address: 172.22.4.4/31} }
  - { name: ae11, descr: 'Q:at1-ce-01:ae1.10:ce', mtu: 1600, unit: {id: 10, address: 172.22.4.32/31, mpls: true} }
  - { name: ae12, descr: 'Q:at1-ce-02:ae1.10:ce', mtu: 1600, unit: {id: 10, address: 172.22.4.34/31, mpls: true} }

# OOB FXP0
  - { name: fxp0, descr: 'M:10.122.128.0/24:mgmt', unit: {id: 0, address: 10.122.128.128/24} }

# LOOPBACK https://netwiki.net.arkadin.lan/resources/ipfabricaddr#loopbacks
  - { name: lo0, descr: 'L:192.168.201.1/32:backbone', unit: {id: 0, address: 192.168.201.1/32} }
  - { name: lo0, descr: 'L:192.168.1.1/32:border', unit: {id: 1, address: 192.168.1.1/32} }
  - { name: lo0, descr: 'L:185.37.223.38/32:inet', unit: {id: 2, address: 185.37.223.38/32} }

# IXRT LEGACY 1G or 10G
  - { name: ae1, descr: 'X:at1-lf-01:ae1.0:fabric', state: enable, mtu: 9234, unit: {id: 500, descr: 'X:atlixrt02:gi0/0/0.500:backbone', address: 172.30.251.253/30, mpls: true}}
...
# 1580 bytes (1500 ethernet payload + 14 Ethernet + 50 VXLAN (8+8+14+20) + 12 MPLS + 4 VLAN).
  - { name: xe-0/1/4, descr: 'E:HE-4333696:chi-bl-01:backbone',
      state: enable, mtu: 1580,
      unit: { id: 0, descr: 'X:chi-bl-01:xe-0/1/4.0', address: 172.30.251.37/30, mpls: true } }

# RPM PROBES
rpm:
  - { owner: 'telemetry', probes: [{ dst_hostname: 'chi-bl-01', src_ip: 172.30.251.37, dst_ip: 172.30.251.38 }]}

# ROUTING-OPTIONS
routing_options:
  mgmt_nexthop: 10.122.128.254  # management's gateway
  router_id: 192.168.201.1      # loopback address
...
protocols:
  ...
  BL_TO_BL: { state: active, ip: 192.168.201.2, descr: 'at1-bl-02' }
  ...
  - name: VRF_ARKAD_INT
    ...
    groups:
      - name: LEAF_V4
        ...
        multipath: true
        neighbors:
          - { ip: 172.22.4.3, descr: 'at1-lf-01', asdot: 1 }
          - { ip: 172.22.4.5, descr: 'at1-lf-02', asdot: 2 }
...
# POLICY-OPTIONS
policy_options:
  loopback_inet: 185.37.223.38/32
  local_prefixes:
